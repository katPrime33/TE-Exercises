...
import com.techelevator.model.ProductWishlist;
...
public class WishlistController {
    ...
    public void addProductToWishlist(@PathVariable("id") long wishlistId,
                                     @RequestBody ProductWishlist productWishlist,
                                     Principal principal) {
        long userId = userDao.findIdByUsername(principal.getName());
        Wishlist wishlist = wishlistDao.get(productWishlist.getWishlistId(), userId);

        if (wishlist != null) {
            wishlistDao.addProductToWishlist(productWishlist.getProductId(),
                    productWishlist.getWishlistId());
        }
    }
}
