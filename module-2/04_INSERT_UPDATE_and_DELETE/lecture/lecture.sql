...
-- Change California's nickname back to "The Golden State", reduce the population by 1,000,000, and change
--the capital back to Sacramento.
UPDATE state
...
INSERT INTO city (city_name, state_abbreviation, population, area)
    VALUES ('Kats City', 'XX', 1, 0.1);

-- Try deleting California from the state table.
DELETE FROM state
WHERE state_abbreviation = 'CA';
--would have to delete all cities in california before deleting the whole state. yeet.
...
-- DETAIL:  Failing row contains (347, Smallville, KS, 0, null).
...
-- Demonstrate two different SQL connections trying to access the same table where one is inside of a
--transaction but hasn't committed yet.
